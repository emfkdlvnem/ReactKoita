        ...
        <section className="login-area relative max-w-set mt-37.5 mx-auto pt-4 px-6 pb-25">
            ...
            <div className="login-box relative w-112.5 h-125 px-12 py-7 mx-auto my-12 rounded-lg shadow-custom">
                <div className="social-area my-5 mx-auto text-center">
                    <button type="button" onClick={handleGoogleLogin} className="relative w-87.5 h-12.5 shadow-md bg-white ">
                        <span>구글 로그인</span>
                    </button>
                </div>
                ...
                <form className="login-form w-87.5 mt-5 mx-auto mb-10" onSubmit={handleLogin}>
                    ...
                    <button type="submit" id="btn-login" className="w-87.5 h-12.5 p-4.5 mb-5 text-white font-bold rounded-lg shadow-md bg-set-brown">로그인</button>
                </form>
                {success && <p className="success">{success}</p>}
                {error && <p className="error">{error}</p>}
                <Link to="/signup" id="btn-signup" className="btn btn-signup-move absolute right-14 bottom-8 py-2 px-3 rounded-lg"><span>회원가입</span></Link>
            </div>
        </section>
    )
}
